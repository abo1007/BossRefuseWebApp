...
export default {
  ...
  methods: {
    ...
    getWorkFaceData() {
      this.$apiList.getWorkface().then(res => {
        if (res.code == 200) {
          this.workfaceData = res.data.data;
    ...
    continueGetWorkFaceData() {
      ...
      if (this.clickLimit) {
        ...
        this.$apiList.getWorkface(this.PageData.current_page + 1).then(res => {
          if (res.code == 200) {
            this.PageData = res.data;

            for (let i = 0; i < res.data.data.length; i++) {
              this.workfaceData.push(res.data.data[i]);
            }
            this.nextUrl = res.data.next_page_url;
            this.clickLimit = true;
          }
    ...
    getBanner() {
      this.$apiList.getBanner().then(res => {
        if (res.code == 200) {
